...
export const legalPages: Record<LegalPageSlug, LegalPageContent> = {
  privacy: {
    ...
    description: "Как RateScope обрабатывает данные пользователей, обращения и технические события сервиса.",
    updatedAt,
    body: `
...
`.trim()
  ...
  terms: {
    ...
    description: "Правила использования RateScope, публичных данных мониторинга и пользовательских материалов.",
    updatedAt,
    body: `
## Общие положения
RateScope предоставляет справочную информацию об обменных направлениях, резервах, репутации и публичных сигналах безопасности. Сервис не является стороной обменной операции.
...
RateScope не гарантирует совершение обмена и не несет ответственность за действия сторонних обменных пунктов. Мы предоставляем инструменты сравнения и публичной проверки, но итоговое решение принимает пользователь.
